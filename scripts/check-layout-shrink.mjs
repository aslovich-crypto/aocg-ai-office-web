...
function checkFile(src) {
  ...
  //
  // ЗАПЯТАЯ БЫЛА ДЫРОЙ (T24). Раньше шаблон требовал `flex: 1,` — с запятой,
  // то есть `style={{ flex: 1 }}` (единственное или ПОСЛЕДНЕЕ свойство
  // в объекте) для сторожа не существовал вовсе. Пять таких мест жили
  // невидимыми, и мутация «убрать minWidth» в одном из них прочиталась как
  // «сторож сломался», хотя ломать было нечего. Теперь граница вместо
  // запятой: `1` не должна быть началом другого числа (flex: 1.5, flex: 10),
  // поэтому после неё разрешены только пробелы и конец свойства.
  //
  // ИСКЛЮЧЕНИЕ ДЛЯ overflow — НЕ ПОБЛАЖКА, А ПРАВИЛО CSS. Автоматический
  // минимальный размер (min-width/min-height: auto), из-за которого элемент
  // не сжимается уже содержимого, применяется ТОЛЬКО когда overflow по этой
  // оси visible. У прокручиваемого элемента он равен нулю, и minWidth:0 ему
  // не нужен. Проверено движком, а не спецификацией по памяти: в строке
  // 200px с несжимаемым соседом 80px элемент `flex:1; overflow:visible`
  // занял 556px (не сжался), а такой же с `overflow:auto` — ровно 120px;
  // в колонке 120px с шапкой 40px тело с overflow:auto село на 80px
  // и получило прокрутку. Исключение сознательно узкое: только шорткат
  // `overflow`, потому что overflowX/overflowY по одной оси оставляют
  // автоминимум по другой, и разобрать это статически нельзя.
  for (const m of src.matchAll(/\bflex:\s*1\s*(?=[,}\n\r])/g)) {
    const obj = objectAround(src, m.index);
    if (/\boverflow:\s*"(auto|scroll|hidden)"/.test(obj)) continue;
