...
window.fetch = (u) => {
  const путь = String(u)
    .replace(/^https?:\/\/[^/]+/, "")
    .split("?")[0];
  // ⚠️ ОДИНОЧНЫЙ ЧЕК ОТДАЁМ НАСТОЯЩИЙ. Карточка перечитывает чек по адресу
  // `/api/receipts/{id}`, а мок отвечал на него пустым массивом — карточка
  // открывалась ПУСТОЙ («Чек 0,00 ₽», «Автор не указан»), и замер про автора
  // проверял пустоту. Скрывал это сам замер: он читал `body.textContent` и
  // находил нужное имя в СВОЁМ ЖЕ исходнике (найдено 05.09.2026).
  const один = путь.match(/^\/api\/receipts\/(\d+)$/);
  const тело = один
    ? ЧЕКИ.find((ч) => ч.id === Number(один[1])) ?? {}
    : ОТВЕТЫ[путь] ?? [];
