...
const СТРУКТУРА = [
  ...
  // NDS-ЧП5 (21.09.2026): сумма НДС по позиции в рублях — из items[].ndsSum,
  // той же дорогой копеек, что price/sum, и ПЕЧАТАЕТСЯ в чипе через money().
  // Обе половины: снять расчёт или снять печать — одинаково красный.
  // Мутация 21.09.2026: `{ndsRub > 0 ? … : vat}` → `{vat}` — поймана.
  [
    "сумма НДС по позиции берётся из it.ndsSum и печатается в чипе",
    () => /Number\(it\.ndsSum\)/.test(т) && /money\(ndsRub\)/.test(т),
  ],
];
let плохо = 0;
console.log("\nСТРУКТУРА");
