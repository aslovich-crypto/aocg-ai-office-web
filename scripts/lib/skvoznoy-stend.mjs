...
// ⚠️⚠️ СТЕНД К ЖИВОЙ 1С НЕ ХОДИТ НИКОГДА (1C-29 ②б, решение владельца
// 22.09.2026, В1). Бэкенд стенда получает окружение оболочки целиком, а в
// оболочке владельца могут лежать настоящие доступы к 1С бюро. От записи
// в чужую бухгалтерию спасала бы только пустая база стенда (нет соответствия
// человека — отказ 409 до обращения к 1С), а это случайность, не защита.
// Поэтому ВСЕ ODATA_* обнуляются явно — и названные, и любые с тем же
// префиксом, — для uvicorn и для питономБэка. Что обнуление работает,
// проверяет драйвер пробы 1С самопроверкой с заведомо мёртвым адресом
// (scripts/check-v1s.mjs), и мутация снимает обнуление — она обязана
// покраснеть.
export const ИМЕНА_1С = [
  "ODATA_URL",
  "ODATA_LOGIN",
  "ODATA_PASSWORD",
  "ODATA_ORG_REF",
...
export function безОбмена1С(окружение) {
  const чистое = { ...окружение };
  for (const к of Object.keys(чистое))
    if (к.startsWith("ODATA_")) чистое[к] = "";
  for (const к of ИМЕНА_1С) чистое[к] = "";
  return чистое;
}
 ...
 * @param {{бэкенд?: Record<string,string>}} настройки — добавка к окружению
 *   бэкенда стенда. ODATA_* она вернуть не может: обнуление накладывается
 *   ПОСЛЕДНИМ.
 */
export async function поднятьСтенд(имяПробы, настройки = {}) {
