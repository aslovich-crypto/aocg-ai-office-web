 ...
 *
 * ТРИ ИСТОЧНИКА, И ПОРЯДОК У НИХ ЖЁСТКИЙ:
 *   ① `vat_breakdown` — чеки ФНС, полная разбивка по тегу 1199;
 *   ② `vat_total` — фото-чеки: «НДС есть, ставка не распознана» (NDS-CLEANUP ②);
 *   ③ `vat_20 + vat_10` — ХВОСТ для чеков, записанных ДО перехода. Уйдёт
 *      вместе с колонками на шаге ③ NDS-CLEANUP, и тогда эта строка удаляется.
 * Складывать их нельзя: у старых чеков ФНС заполнены и разбивка, и колонки,
 * и сумма дала бы двойной счёт — ошибку той же природы, что чиним, только
 * в другую сторону.
 */
export function receiptVat(r) {
  ...
  const всего = Number(r?.vat_total) || 0;
  if (всего > 0) return всего;
