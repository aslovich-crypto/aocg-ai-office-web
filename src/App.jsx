...
// ⚠️ 22.09.2026 (1C-29 ②а): отправка в 1С ПОЯВИЛАСЬ — ручки в
// `app/routers/odata.py` (бэкенд 632b71c) и кнопка «Отправить в 1С» в карточке
// одобренного отчёта. Слова вернулись в подпись ТЕМ ЖЕ КОММИТОМ, что и
// кнопка: обещание появляется вместе с тем, что его выполняет, не раньше.
//
// ЧЕМ МЕРЕНО, ПОГЕЙТНО:
...
//   _бухгалтер_или_администратор → отправка отчёта в 1С, её отмена и
//                               состояние (тот же круг, can_see_all)
...
const ROLES = [
  ...
  },
  {
    id: "accountant",
    label: "Бухгалтер",
    desc: "Видит все чеки и отчёты организации, одобряет и отклоняет их, отправляет одобренные в 1С, ведёт справочники категорий и карт. Людьми не управляет.",
  },
  {
    id: "admin",
    label: "Администратор",
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button
            onClick={() =>
              // ⚠️ ПЕРЕДАЁМ ВСЁ, ЧТО ЧЕЛОВЕК НАБРАЛ, А НЕ ТОЛЬКО ДАТУ И СУММУ.
              // Раньше уходили две величины из шести, и реквизиты с бумажного
              // чека гибли ровно в том сценарии, ради которого их и вводили:
              // ФНС не подтвердила — значит чек идёт в базу руками, и ФН/ФД/ФПД
              // это единственное, чем его потом опознать (строка 31, правка ②).
              onManualFallback({ date, time, amount, fn, fd, fpd })
            }
            style={fallbackBtn}
          >
            {errMsg
...
function OperaciiPage({
  ...
  const [form, setForm] = useState({
    ...
    date: todayISO(),
    fn: "",
    // ⚠️ ФД, ФПД И ВРЕМЯ ЖИВУТ В ФОРМЕ С 12.09.2026 (строка 31). Раньше их
    // здесь не было вовсе: разбор QR их отдавал, а форма роняла — и чек
    // уезжал в базу без реквизитов, хотя фронт держал их в руках.
    fd: "",
    fpd: "",
    time: "",
    raw_data: null,
  ...
  async function handleCapture(qrText) {
    ...
    setForm((p) => ({
      ...
      category: "Не указано",
      fn: parsed.fn || "",
      // Три значения, снятые с НАПЕЧАТАННОГО QR, а не распознанные: довод
      // «распознанной цифре в реквизите не место» к ним не относится.
      fd: parsed.fd || "",
      fpd: parsed.fpd || "",
      time: parsed.time || "",
      raw_data: null,
  ...
  function openManualForm(prefill) {
    ...
    setForm((p) => ({
      ...
      org: "",
      category: "Не указано",
      // ⚠️ БЫЛО `fn: ""` — набранное человеком затиралось ЯВНОЙ строкой, а не
      // терялось по недосмотру. Теперь переносим: реквизиты с бумажного чека
      // и есть то, чем чек без ответа ФНС потом опознают (строка 31, ②).
      fn: prefill?.fn || "",
      fd: prefill?.fd || "",
      fpd: prefill?.fpd || "",
      time: prefill?.time || "",
  ...
  async function addR() {
    ...
    try {
      ...
      // ⚠️ ФД И ФПД ШЛЮТСЯ С 12.09.2026, И ЭТО ГЛАВНАЯ ПРАВКА СТРОКИ 31.
      // Бэкенд принимал их с 31.08.2026 (T132, шаг ⓪ — заведено ровно под
      // случай «ФНС промолчала»), а фронт не слал НИ РАЗУ: канал открыли
      // и не подключили. Замер владельца на проде 11.09.2026 — реквизиты
      // есть у 22 чеков из 98.
      // ⚠️ ЭТО НЕ ТО ЖЕ, ЧТО `ocr_fd`/`ocr_fpd` НИЖЕ. Те распознаны моделью
      // и служат ТОЛЬКО признаком дубля; эти сняты с напечатанного QR либо
      // набраны человеком с бумаги — им в реквизитах место.
      if (form.fd) payload.fd_num = form.fd;
      if (form.fpd) payload.fpd = form.fpd;
      // Время чека: без него бэкенд не вернёт чеку право на дозапрос —
      // строка запроса к ФНС собирается из пяти значений, и время одно из них.
...
export default function App() {
  ...
  // ⚠️ ОТКАЗ СЕРВЕРА БОЛЬШЕ НЕ ВЫДАЁТСЯ ЗА ПУСТОТУ (T171, седьмой источник,
  // найден аудитом 10.09.2026). Здесь стояло `if (!res.ok) return;` с
  // припиской «колокольчик не повод для тоста» — и человек видел «Пока
  // ничего нового», когда список НЕ ЗАГРУЗИЛСЯ. Ровно та беда, которую
  // коммит `9bebfbf` лечил по шести источникам: молчание вместо отказа.
  //
  // ⚠️ ПОЧЕМУ ЗАХОД ПО T171 ПРОШЁЛ МИМО — это важнее самой правки. Опись
  // источников составлялась по тому, ЧТО ИСКАЛИ: `.catch(() => {})`.
  // А здесь отказ проглатывается ДРУГОЙ формой — ранним `return` по
  // `!res.ok`. Один класс, две записи в коде; поиск знал одну.
  //
  // ⚠️ ПРИПИСКА ПРО ТОСТ БЫЛА ВЕРНА И ОСТАЁТСЯ: тост тут и правда не нужен.
  // Но «не тост» не значит «молчать» — беда уходит в общую плашку под
  // шапкой (`сбоиЗагрузки`), туда же, где живут чеки, люди и роль.
  async function перечитатьУведомления() {
    try {
      const res = await authFetch("/api/notifications/");
      if (!res.ok) {
        пометитьСбой("уведомления", true);
        return;
      }
      const d = await res.json();
      setУведомления(Array.isArray(d.items) ? d.items : []);
      setНепрочитано(Number(d.unread) || 0);
      пометитьСбой("уведомления", false);
    } catch {
      // Офлайн — прежнее число оставляем (оно не врёт, оно устарело),
      // но об отказе говорим: иначе пустой список читается как «нового нет».
      пометитьСбой("уведомления", true);
    }
  }
  ...
  async function открытьУведомления() {
    ...
    if (непрочитано > 0) {
      setНепрочитано(0);
      try {
        // ⚠️ ШЛЁМ ID ТОГО, ЧТО НАРИСОВАЛИ, А НЕ «погаси всё» (10.09.2026).
        // Раньше тело было пустым, и сервер гасил ВСЁ непрочитанное — при
        // двадцати пяти событиях пять помечались прочитанными, не побывав
        // на экране: список отдаёт двадцать. Непрочитанное значит «человек
        // не видел», и пометить невыведенное — потерять уведомление
        // навсегда. Клиент знает, что нарисовал, — он и говорит.
        const res = await authFetch("/api/notifications/read", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: уведомления.filter((н) => !н.read).map((н) => н.id),
          }),
        });
        // ⚠️ УСПЕХ POST ПРОВЕРЯЕТСЯ, А НЕ ПРЕДПОЛАГАЕТСЯ (аудит 10.09.2026).
        // `authFetch` возвращает ответ и на отказе тоже — без этой проверки
        // 500 от сервера проходил как успех, точка гасла навсегда, а на
        // сервере события оставались непрочитанными.
        if (!res.ok) пометитьСбой("уведомления", true);
      } catch {
        пометитьСбой("уведомления", true);
      }
    }
    // ⚠️ ПЕРЕЧИТЫВАЕМ ВСЕГДА, А НЕ ТОЛЬКО ПРИ НЕПРОЧИТАННЫХ. Раньше и POST,
    // и GET лежали внутри одного `if`: при нуле непрочитанных открытие
    // шторки показывало то, что пришло при последней смене экрана, — то есть
    // человек мог смотреть на устаревший список и не знать об этом.
    перечитатьУведомления();
  }

  // ── ОТМЕНА ДЕЙСТВИЯ СНЯТА 22.09.2026 (REP-EXPDEL ②, решение владельца В4).
  // Тост «Отменить» с отложенным запросом жил здесь ради ОДНОГО места —
  // удаления отчёта свайпом. С этого захода отчёт спрашивает подтверждение
  // ДО удаления (`ConfirmDeleteReportSheet`, одна шторка на список и
  // карточку), и у механизма не осталось ни одного вызывающего. Мёртвый
  // код в оболочке опаснее отсутствующего: он выглядит работающим и
  // обещает страховку, которой нет, — поэтому снят целиком, а не оставлен
  // «на будущее» (класс T149, «сделали и не подключили»).
