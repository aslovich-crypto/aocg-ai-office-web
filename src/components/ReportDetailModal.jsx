...
import RenameReportSheet from "./RenameReportSheet";
...
export default function ReportDetailModal({
  ...
  const [спроситьИмя, setСпроситьИмя] = useState(false);
  const [переименовывается, setПереименовывается] = useState(false);
  ...
  // ⚠️ ПЕРЕИМЕНОВАНИЕ — ТОЛЬКО ДО ОТПРАВКИ В 1С (REP-RENAME, 23.09.2026).
  // Название уезжает В ДОКУМЕНТ 1С и остаётся снимком в журнале выгрузок:
  // переименовав отчёт после отправки, мы развели бы наше имя и имя в чужой
  // бухгалтерии, и человек, ищущий документ по названию, не нашёл бы его.
  // Круг прав тот же, что у удаления, и по тому же доводу — это правка
  // документа, который уже смотрят чужими глазами.
  const можноПереименовать =
    !!onStatus &&
    !живаяОтправка &&
    (role != null && canApprove(role)
      ? true
      : rep.status === "Черновик" || rep.status === "Отклонён");
  const естьМеню =
    можноУдалить || можноОтменить1С || можноОтозвать || можноПереименовать;
  ...
  async function отменить() {
    ...
    if (!о.ок) setОшибкаВыгрузки(о.текст || НЕТ_ОТВЕТА);
    // ⚠️ ПОМЕТИТЬ НЕ ВЫШЛО — ЧЕЛОВЕК ОБЯЗАН ЭТО УЗНАТЬ ([[1C-DELMARK]], 23.09.2026).
    // Отмена у нас проходит всегда, даже когда 1С отказала или недоступна;
    // молча проглотить это значило бы оставить документ в чужой бухгалтерии
    // и никому не сказать. Текст — СЕРВЕРА, дословно: причина известна ему.
    else if (о.тело && о.тело.помечен === false)
      setОшибкаВыгрузки(о.тело.заметка || "");
    setКлюч1С((к) => к + 1);
  }

  // ⚠️ ПОСЛЕ СОХРАНЕНИЯ БЕРЁМ ОТВЕТ СЕРВЕРА, А НЕ СВОЁ ЗНАЧЕНИЕ (класс T156).
  // Сервер срезает пробелы по краям и может отказать; подставив своё, мы
  // показали бы имя, которого в базе нет, — ровно тот случай, когда рядом
  // живут два источника одного факта и побеждает не тот.
  async function переименовать(имя) {
    if (переименовывается) return;
    setПереименовывается(true);
    setОшибкаВыгрузки("");
    try {
      const res = await authFetch(`/api/reports/${rep.id}/title`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: имя }),
      });
      const тело = await res.json().catch(() => null);
      if (!res.ok) {
        // Текст отказа приходит с бэкенда готовым: 403 про права, 409 про
        // отправку в 1С, 422 про пустое и слишком длинное имя.
        setОшибкаВыгрузки(текстОшибки(тело, "Переименовать не удалось"));
        return;
      }
      setFull((п) => ({ ...(п || {}), ...тело }));
      if (onChanged) onChanged(тело);
      setСпроситьИмя(false);
    } catch {
      setОшибкаВыгрузки("Сервер не ответил. Название не изменилось");
    } finally {
      if (aliveRef.current) setПереименовывается(false);
    }
  }
    ...
    >
      ...
      >
        ...
        {спроситьИмя && (
          <RenameReportSheet
            отчёт={rep}
            занято={переименовывается}
            onKeep={() => !переименовывается && setСпроситьИмя(false)}
            onConfirm={переименовать}
          />
        )}
