...
// ⚠️ ИМЯ И ТЕКСТ ОШИБКИ — ПЕРВОЙ СТРОКОЙ, ВСЕГДА. Первая редакция брала
// `ошибка.stack`, и в Chrome этого хватало: там стек НАЧИНАЕТСЯ строкой
// «TypeError: …». **В Safari стек — только кадры, без сообщения.** Владелец
// прислал стек с iPhone 31.08.2026, и самой ошибки в нём не было ни слова:
// прибор показал, ГДЕ, но не ЧТО. Теперь заголовок собирается сам, а стек
// идёт следом — и от браузера это больше не зависит.
function заголовокОшибки(ошибка) {
  if (!ошибка) return "Неизвестная ошибка";
  if (typeof ошибка === "string") return ошибка;
  const имя = ошибка.name || "Error";
  const текст = ошибка.message || String(ошибка);
  return текст.startsWith(имя) ? текст : `${имя}: ${текст}`;
}

function собратьПричину(ошибка, сведения) {
  const части = [];
  части.push(заголовокОшибки(ошибка));
  const стек = ошибка && ошибка.stack ? String(ошибка.stack) : "";
  if (стек) части.push(стек);
  const дерево = сведения && сведения.componentStack;
  if (дерево) части.push("Компоненты:" + дерево);
  части.push(
...
export default class ErrorBoundary extends Component {
  ...
  static getDerivedStateFromError(ошибка) {
    // ⚠️ Тоже с заголовка: если componentDidCatch по какой-то причине
    // не отработает, человек всё равно увидит, ЧТО сломалось.
    return {
      причина:
        заголовокОшибки(ошибка) +
        (ошибка && ошибка.stack ? "\n\n" + ошибка.stack : ""),
    };
  }
