...
export default function ReceiptDetailModal({
  ...
  // Старые колонки vat_20/vat_10 в БД остаются. ЗДЕСЬ читается только breakdown —
  // но «фронт читает только breakdown» было бы неправдой: до 10.08.2026 входящий
  // НДС в src/lib/tax.js считался как vat_20+vat_10 и не видел ставку 22
  // (занижение 70,5% по замеру прода). Утверждение о СОСЕДНЕМ файле, написанное
  // здесь, полтора месяца выглядело фактом — поэтому теперь тут сказано только
  // про этот файл, а общий переход отслеживается задачей NDS-CLEANUP. ──
  const VAT_ORDER = ["22", "20", "10", "7", "5", "0"];
  const bd = r.vat_breakdown || {};
  const vatRows = VAT_ORDER.filter((rate) => Number(bd[rate]) > 0).map(
    (rate) => [`НДС ${rate}%`, Number(bd[rate])],
  );
            ...
            {items.length > 0 && (
              <section style={{ ...blockStyle, padding: "14px 16px 6px" }}>
                ...
                {items.map((it, i) => {
                  ...
                  // бейдж — любая реальная ставка (20/10/22/5/7/0/расчётные), кроме «Без НДС»
                  const showVat = vat !== "" && vat !== "Без НДС";
                  return (
                    <div
                      key={i}
                      style={{
