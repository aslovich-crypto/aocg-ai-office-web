...
export default function ScanReceiptModal({
  ...
  async function usePhoto(e) {
    ...
    // ⚠️ ТЕКСТ ПРАВЛЕН 29.08.2026: ПРЕЖНИЙ СОЗДАВАЛ КОНФЛИКТ У КАССЫ.
    // Было: «Возможно, чек старше 30 дней или НЕ ЗАРЕГИСТРИРОВАН».
    // Обе причины неверны для СВЕЖЕГО чека — а он и будет самым частым,
    // когда с 01.09 сканируют сотрудники. Настоящая причина: данные
    // с кассы идут в налоговую от нескольких часов до суток.
    //
    // ⚠️ «Не зарегистрирован» хуже просто неверного: читается как
    // ОБВИНЕНИЕ ПРОДАВЦА и посылает человека ругаться с кассиром вместо
    // того, чтобы подождать. Незарегистрированный чек — редкость;
    // свежий будет каждый день. Убрано.
    //
    // ⚠️ «30 дней» осталось БЕЗ ЧИСЛА: порог не наш, он на стороне ФНС,
    // и называть его значит обещать за налоговую.
    //
    // ⚠️ Заголовок «не найден» → «ещё не дошёл»: первое про поиск
    // и читается как «его там нет», второе про время и сразу говорит,
    // что делать. Всю разницу несёт слово «ещё».
    //
    // ⚠️ «ФНС временно недоступна» НЕ ТРОГАЛСЯ — там речь о сбое,
    // а не о времени, и текст верен.
    //
    // ⚠️ ТЕКСТ ПОВТОРЁН В ИНСТРУКЦИИ: docs/onboarding-employee.md
    // репозитория aocg-ai-office, таблица «Если что-то не так».
    // Меняется здесь — меняется и там, одним коммитом. Сторож
    // scripts/check-scan-texts.mjs сверяет обе стороны.
    setSaveSheet({
      title: unavailable
        ? "ФНС временно недоступна"
        : "Чек ещё не дошёл до ФНС",
      message: unavailable
        ? "Не удалось проверить чек через ФНС. Сохранить как фото? Позже можно проверить вручную."
        : "Данные с кассы идут в налоговую от нескольких часов до суток. Если чек старше — проверьте номер на бумаге. Сохранить как фото прямо сейчас?",
