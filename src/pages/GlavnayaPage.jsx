...
// В КОМПАКТНОМ СПИСКЕ «ПОСЛЕДНИЕ» ОТ СПОСОБА ОПЛАТЫ НУЖНЫ ТОЛЬКО ЦИФРЫ.
// Замер 03.08.2026 @320 после починки ширин: имени карты доставалось 40.6px
// из нужных 135 — на экране «Кор…». Это не идентифицирует карту: то, что это
// карта, уже сказано иконкой, а какая именно — не прочесть. Последние четыре
// цифры — единственное, что различает карты между собой. Ширина «•3950»
// измерена в самом элементе: 41.0px — под неё поднят пол левой колонки
// (148 → 152), см. комментарий у самого пола. Полное имя остаётся в деталях
// чека. ТОЛЬКО ЗДЕСЬ: на «Чеках» и в деталях своя ширина и свои решения.
//
// ОТКУДА ЦИФРЫ — порядок неслучаен, он выведен из данных прода, а не из схемы:
//  1. receipts.card_last4. Колонку заполняют парсеры ФНС и OCR, когда номер
//     виден в самом чеке. НА 03.08.2026 ОНА ПУСТА У ВСЕХ 29 ЧЕКОВ ПРОДА
//     (28 qr_scan + 1 photo_ocr): в наших данных карта приходит не из чека,
//     а из справочника организации. Первой всё равно проверяем её — это
//     единственный источник, где цифры взяты из документа, а не из имени.
//  2. Хвост из четырёх цифр в названии карты (receipts.payment хранит имя из
//     справочника): «Корпоративная 3950», «Личная 6645» — так названы обе
//     карты организации, покрытие 29 из 29.
//  3. Цифр нет вовсе — «Наличные», «Не указано», карта без номера в имени.
//     Тогда показываем НАЗВАНИЕ, как было: короткие значения влезают целиком,
//     а пустое место хуже огрызка.
function paymentShort(r) {
  ...
  return digits ? `•${digits}` : r.payment || "—";
}
            ...
            return (
              ...
              >
                {/* ПОЛ ЛЕВОЙ КОЛОНКИ — 152px. Число выведено из содержимого,
                    ...
                    никогда), плюс иконка карты 14 и её отступ 4, плюс 41.0px —
                    ширина «•3950», ИЗМЕРЕННАЯ В САМОМ ЭЛЕМЕНТЕ. Итого 148.4,
                    округлено вверх до 152. Порог тот же по смыслу, что
