...
import { BADGE, canApprove } from "../lib/reports";
...
export default function OtchetyPage({
  ...
  useEffect(() => {
    ...
    const т = setTimeout(() => {
      openCreate();
      if (onReportSignalConsumed) onReportSignalConsumed();
    }, 0);
      ...
      {filtered.length === 0 ? (
        ...
        >
          {filtered.map((rep) => {
            ...
            // МАТРИЦА ЖЕСТОВ — ПО КАНОНУ templates/reports (03.09.2026, «ок»
            // владельца), с двумя ИМЕНОВАННЫМИ отступлениями:
            //   Черновик    → Отправить · Удалить      (канон дословно)
            //   На проверке → Отозвать                 (канон: + «Открыть»)
            //   Отклонён    → Исправить · Удалить      (канон: Исправить · Открыть)
            //   Одобрен     → жеста нет                (канон: «Открыть»)
            // ⚠️ ОТСТУПЛЕНИЯ.ОтчётыСвайп-Открыть: «Открыть» жестом НЕ дублируем
            //   нигде — тап по строке уже открывает; два пути к одному действию
            //   выпалываем весь месяц (T104). Решение владельца.
            // ⚠️ ОТСТУПЛЕНИЯ.ОтчётыСвайп-УдалитьОтклонённый: «Удалить» у
            //   Отклонённого канон не рисует, мы оставляем — отчёт с ошибочными
            //   чеками легитимно выбрасывают; канонная карточка просто не
            //   вместила три полосы. Решение владельца.
            //
            // ⚠️ ПРЕЖНИЙ ЗАМЕР 05.08 ЗАПРЕЩАЛ ВТОРУЮ КНОПКУ — вывод снят,
            // числа были верны: панель 2×84 действительно накрывает сумму
            // и бейдж на всех ширинах (320→430). Но канон САМ рисует открытую
            // панель с двумя полосами поверх правого края: скрытие при жесте —
            // свойство свайпа, а не поломка; сумма видна ДО жеста, во время
            // жеста человек смотрит на кнопки. Прежняя трактовка мерила жест
            // меркой покоя. Снятие вывода доложено владельцу, не молча.
            //
            // «Отозвать» без тоста-отмены: действие обратимо штатно —
            // «Отправить» вернёт на проверку. Тост держим для необратимого.
            const actions =
                  ...
                  ? [
                      {
                        ...
                        label: "Отправить",
                        Icon: Send,
                        bg: "#15803D", // sa-send канона
                        onPress: () => changeStatus(rep.id, "На проверке"),
                      },
                      {
                        key: "del",
                        label: "Удалить",
